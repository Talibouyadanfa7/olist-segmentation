...
def make_dataset(dpath="olist_datasets/", initial=False, period=2):
    ...
    """
    start_time = time()
    logger.info("Création du dataset en cours ...")
    ...
    products['product_category'] = np.where((products['product_category_name'].str.contains("fashio|luggage") == True),
                                            ...
                                            np.where((products['product_category_name'].str.contains(
                                                "health|beauty|perfum") == True),
                                                     'health_beauty',
                                                     np.where((products['product_category_name'].str.contains(
                                                         "toy|baby|diaper") == True),
                                                              'toys_baby',
                                                              np.where((products['product_category_name'].str.contains(
                                                                  "book|cd|dvd|media") == True),
                                                                       'books_cds_media',
                                                                       np.where((products[
                                                                                     'product_category_name'].str.contains(
                                                                           "grocer|food|drink") == True),
                                                                                'groceries_food_drink',
                                                                                np.where((products[
                                                                                              'product_category_name'].str.contains(
                                                                                    "phon|compu|tablet|electro|consol") == True),
                                                                                         'technology',
                                                                                         np.where((products[
                                                                                                       'product_category_name'].str.contains(
                                                                                             "home|furnitur|garden|bath|house|applianc") == True),
                                                                                                  'home_furniture',
                                                                                                  np.where((products[
                                                                                                                'product_category_name'].str.contains(
                                                                                                      "flow|gift|stuff") == True),
                                                                                                           'flowers_gifts',
                                                                                                           np.where((
                                                                                                                   products[
                                                                                                                       'product_category_name'].str.contains(
    ...
    logger.info(54 * "_")
    logger.info("Features complétées avec la valeur la plus fréquente :")
    logger.info(54 * "_")
    for f in features_to_fill:
        data[f] = data[f].fillna(data[f].mode()[0])
        logger.info(f, "\t", data[f].mode()[0])
    logger.info(54 * "_")

    end_time = time()
    logger.info("Durée d'execution du Feature engineering : {:.2f}s".format(end_time - start_time))
